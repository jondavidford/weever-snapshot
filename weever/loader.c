...
static unsigned char *next = (unsigned char*) 0x7a7a7c;
...
static unsigned char hexify(unsigned char x)
{
  return (x<10 ? x+'0' : (x-10)+'a');
}

static void dump(char name[4], unsigned char *buf, int n)
{
   print_status('P','F');
   *next++ = *name++;
   *next++ = *name++;
...
#define DUMP4(name,var) dump(name,(unsigned char*) &(var),4);
#define DUMP8(name,var) dump(name,(unsigned char*) &(var),8);
...
/*
    *(uint64_t*)(0x7c7c70) = (uint64_t)loader;
    
    char * tmp = (char*)(uint64_t)(bp->cmdline_ptr);
    int j;
    for (j = 0; j < bp->cmdline_size; j+=2) {
        print_status(*(tmp+j), *(tmp+j+1));
    }
*/
...
*/
    ...
    DUMP8("lina",linear_alloc);
    DUMP8("kems",kernel->mod_start);
    DUMP8("kent",kernel_entry);
    DUMP8("*ker",*kernel);
